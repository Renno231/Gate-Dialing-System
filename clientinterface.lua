...
local SettingsFile = "/gds/gateEntries.cfg"
...
local settings = { --add a read and save ability
    ...
    modemRange = 200;
    prefix = ";";
    autoLogToDatabase = true;
    outputBufferSize = 120;
...
--reading and applying local settings
...
local function scan()
    return broadcast(settings.networkPort, string.format('gds{command = "query", user = {name = %q}}', lastUser))
end
...
local dialButton = buttonapi.Button.new(gateOperator.pos.x+7, nearbyGatesList.pos.y + 5, 1, 1, "Dial", 
function() 
    if databaseList.currententry and nearbyGatesList.currententry then
        ...
        processInput("", settings.prefix.."dial "..gateA.." "..gateB) 
    end
end)
...
historyTab.func = function() 
    databaseList.visible, nearbyGatesList.visible, dialButton.visible, scanNearbyButton.visible = false, false, false, false 
    gateOperator:clear()
    historyList:display()
end
...
local commands = {
    ...
    dial = function(...)
        ...
        if gateA and gateB then
            if gateA.UUID=="unknown" or gateA.UUID == nil then
                ...
                send(gateA.UUID, settings.networkPort, cmdPayload..serialization.serialize(gateB.Address):sub(1).."}}")
    ...
    close =  function(...)
        ...
        if gateA then
            send(gateA.UUID, settings.networkPort, 'gds{command="close"}')
...
local EventListeners = {
    modem_message = event.listen("modem_message", function(_, receiver, sender, port, distance, msg)
        if type(msg) == "string" then
            if msg:sub(1, 4) == "gds{" and msg:sub(msg:len()) == "}" and msg:len() > 10 then
                recordToOutput("Receiving data..")
                local msgdata = load("return "..msg:sub(4))() --{gateType, address = {MW = ..., }, uuid = modem.address}
